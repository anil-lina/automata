...
def build_oracle_asset(table_name, is_incremental):
    ...
    def _oracle_asset(context):
        ...
        if is_incremental:
            ...
            query = f"SELECT * FROM {table_name} WHERE PROCESSED_DTTM > TO_TIMESTAMP('{last_timestamp}', 'YYYY-MM-DD HH24:MI:SS')"
        else:
        ...
        if is_incremental and not df.empty:
            max_timestamp = df["PROCESSED_DTTM"].max()
...
def load_oracle_assets():
    ...
    return [build_oracle_asset(config["name"], config["incremental"]) for config in table_configs]

...
def sync_asset(context: AssetExecutionContext, config: SyncConfig, oracle: oracledb.Connection, oci: ObjectStorageClient):
    ...
    for table_info in tables_config.get("tables", []):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            while True:
                ...
                if is_incremental and incremental_column and data:
                    current_max = max(row[incremental_column] for row in data)
                    if max_incremental_value is None or current_max > max_incremental_value:
                ...
                context.log.info(f"Uploaded {file_name} to OCI bucket {oci_bucket}.")
